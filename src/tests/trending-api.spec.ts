...
describe("Trending GIFs API", () => {
    let response: Response;
    let body: Array<{ [key: string]: unknown }>;

    beforeAll(async () => {
        response = await fetch("https://api.giphy.com/v1/gifs/trending?api_key=3IFw2ECzX0WpG6D3bAE6SY2IQRBVgsZL&limit=10&offset=0&rating=g");
        body = await response.json();
    });

    test("base url should be correct", () => {
        expect(response.url).toBe("https://api.giphy.com/v1/gifs/trending?api_key=3IFw2ECzX0WpG6D3bAE6SY2IQRBVgsZL&limit=10&offset=0&rating=g");
    });
